...
const encodeFromS3 = (bucket, id, subtitleFile, inputPath, outputPath, job) => {
  job.progress(
    0,
    100,
    `Subtitling ${inputPath} with ${subtitleFile} to ${outputPath}`
  );
  return getS3Json(bucket, id).then(
...
const subtitleMovie = (bucket, id, job) => {
  ...
  return downloadSubs(youtubeUrl, vttFilename, job)
    .then(filename =>
      encodeFromS3(bucket, id, filename, `${id}.mp4`, `${id}_subbed.mp4`, job))
