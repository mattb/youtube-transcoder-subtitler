...
const getS3 = path =>
  s3
    .getObject({ Bucket: s3Bucket, Key: path })
    .promise()
    .then(result => JSON.parse(result.Body.toString()));

const getAllMetadata = () => listS3()
  .then(list => list.filter(file => file.Key.endsWith('.json')))
  .then(files => Promise.all(files.map(file => getS3(file.Key))));

const getMetadata = id => getS3(`${id}.json`);

const putMetadata = (id, data) => s3.putObject({
  Bucket: s3Bucket,
  ACL: 'public-read',
  Key: `${id}.json`,
...
const deleteS3 = id => {
  const filenames = [`${id}.mp4`, `${id}.json`, `${id}_subbed.mp4`];
